...
RUN yum install -y openssh-server xterm git mysql mysql-server httpd24-httpd php55w php55w-common php55w-cli mysql mysql-server php55w-devel php55w-mcrypt php55w-mbstring php55w-mysqlnd php55w-pdo php55w-pecl-xdebug php55w-xml php55w-xmlrpc php55w-opcache php55w-gd php55w-pecl-imagick
...
RUN sed -i 's/Options Indexes FollowSymLinks/Options All Indexes FollowSymLinks MultiViews/' /etc/httpd/conf/httpd.conf
